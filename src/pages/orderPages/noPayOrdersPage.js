...
import { 
 ...
 class NoPayOrdersPage extends React.Component {
   constructor() {
     super()
     this.state = {
       dataList: [],
   ...
   componentDidMount() {
     this.props.payOrders({status:0})
   }
   renderCell(data) {
    const order = data.item
    return (
     <TouchableHighlight key={order.id} onPress={()=>{this.props.navigation.navigate('NoPayOrderDetail',{orderCode:order.orderCode})}}>
       <View>
   ...
   onHeaderRefresh(){
    ...
    this.props.payOrders({status:0,pageNo:1},(RefreshState)=>{this.setState({refreshState: RefreshState})})
  }
onFooterRefresh() {
 ...
 export default NoPayOrdersPage